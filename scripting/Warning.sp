		...
		switch (action) {
			case MenuAction_Select:
			{
				...
				for (int i = 1; i <= playercount;i++) 
				{
					...
					char idtest[40];
					GetClientAuthId(targetid, AuthId_Steam2, idtest, sizeof(idtest));
					
					char query[100];
					Format(query, sizeof(query), "SELECT Reason FROM Warnings WHERE SteamID = '%s'", idtest);
